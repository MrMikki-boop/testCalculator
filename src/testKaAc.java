...
public class testKaAc {
    ...
    public static String calc(String input) {
        ...
        char[] uChar = new char[10]; // Создание массива из символов с размером 10
        ...
        switch (opera) {
            ...
            case '/' -> {
                try { //  служит для определения блока кода, в котором может произойти исключение
                    result = num1 / num2;
                } catch (Exception e) { // необходим для определения блока кода, где происходит обработка исключения
                    throw new RuntimeException("Нельзя делить на ноль");
                }
            }
            default -> throw new RuntimeException("Неверная операция"); // throw исключение
        }
        return result;
    }
